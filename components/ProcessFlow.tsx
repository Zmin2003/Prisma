...
import { Users, Zap, Brain, Loader2, CheckCircle2, Clock, AlertTriangle, TrendingDown } from 'lucide-react';
import { AppState, AnalysisResult, ExpertResult, DegradationStatus } from '../types';
import { DEGRADATION_CONFIG, DEEPTHINK_TIMEOUTS } from '../config';
...
const DegradationBanner = ({ status }: { status: DegradationStatus | null | undefined }) => {
  if (!status) return null;

  const levelConfig = DEGRADATION_CONFIG.LEVELS[status.level];
  const reasonText = {
    timeout: 'Timeout detected',
    retry_exhausted: 'Retry limit reached',
    api_error: 'API error',
    network_error: 'Network error',
    manual: 'Manual degradation'
  }[status.reason || 'manual'];

  const levelColors = {
    full: 'bg-blue-50 border-blue-200 text-blue-900',
    simplified: 'bg-amber-50 border-amber-200 text-amber-900',
    single: 'bg-orange-50 border-orange-200 text-orange-900',
    direct: 'bg-red-50 border-red-200 text-red-900'
  };

  const iconColors = {
    full: 'text-blue-600',
    simplified: 'text-amber-600',
    single: 'text-orange-600',
    direct: 'text-red-600'
  };

  return (
    <div className={`mb-4 ${levelColors[status.level]} border rounded-lg p-3 flex items-start gap-3`}>
      <div className="flex items-center gap-2">
        <AlertTriangle size={18} className={`${iconColors[status.level]} mt-0.5 flex-shrink-0`} />
        <TrendingDown size={18} className={`${iconColors[status.level]} mt-0.5 flex-shrink-0`} />
      </div>
      <div className="flex-1 text-sm">
        <div className="font-semibold">
          Degraded Mode: {levelConfig.name}
        </div>
        ...
        <div className="text-xs mt-2 flex items-center gap-4">
          ...
          {status.originalLevel && status.originalLevel !== status.level && (
            <span className="px-2 py-0.5 bg-white/50 rounded border border-current/20">
              Original: {DEGRADATION_CONFIG.LEVELS[status.originalLevel].name}
            </span>
          )}
        </div>
      </div>
...
const ProcessFlow = ({ appState, managerAnalysis, experts, degradationStatus, defaultExpanded = true, processStartTime, processEndTime }: ProcessFlowProps) => {
  ...
  return (
    <div className="relative space-y-4 pt-4 w-full">
      ...
      <DegradationBanner status={degradationStatus} />
      ...
      <div className="relative space-y-2">
        ...
        <ProcessNode
          ...
          timeoutMs={managerStatus === 'active' ? DEEPTHINK_TIMEOUTS.MANAGER : undefined}
          startTime={managerStatus === 'active' ? processStartTime || undefined : undefined}
        >
        ...
        {hasExperts && (
          <ProcessNode
            ...
            isExpanded={isExpanded}
            onToggle={() => setIsExpanded(!isExpanded)}
            timeoutMs={expertsStatus === 'active' ? DEEPTHINK_TIMEOUTS.EXPERT : undefined}
            startTime={expertsStatus === 'active' ? experts.find(e => e.status === 'thinking')?.startTime : undefined}
          >
        ...
        {(isSynthesisActive || isComplete) && (
          <ProcessNode
            ...
            isExpanded={isExpanded}
            onToggle={() => setIsExpanded(!isExpanded)}
            timeoutMs={isSynthesisActive ? DEEPTHINK_TIMEOUTS.SYNTHESIS : undefined}
            startTime={isSynthesisActive ? processStartTime || undefined : undefined}
          >
