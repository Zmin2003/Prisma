      ...
      <div className="flex items-center justify-between">
       ...
       <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg flex items-center justify-between">
          <div className="flex items-center gap-2">
             ...
             <div>
                <p className="text-sm font-medium text-indigo-900">Process Stream</p>
                <p className="text-[10px] text-indigo-600/80">Enable real-time reasoning process updates via WebSocket.</p>
             </div>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input 
              type="checkbox" 
              checked={config.enableProcessStream ?? true} 
              onChange={(e) => setConfig({ ...config, enableProcessStream: e.target.checked })} 
