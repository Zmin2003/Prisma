...
import { ModelOption, AnalysisResult, ExpertResult, ReviewResult, MessageAttachment, TimeoutError } from '../../types';
...
export const executeManagerAnalysis = async (
  ...
  const analysisTask = async (): Promise<AnalysisResult> => {
    ...
    if (isGoogle) {
      ...
      try {
        const analysisResp = await withRetry(
          ...
          RETRY_CONFIG.NETWORK_ERROR.initialDelay,
          RETRY_CONFIG.NETWORK_ERROR.backoffMultiplier,
          (status) => {
            logger.info("Manager", `Retry attempt ${status.attempt}/${status.maxRetries}`, {
  ...
  try {
    return await withTimeout(analysisTask(), DEEPTHINK_TIMEOUTS.MANAGER, 'manager');
  } catch (e) {
    if (e instanceof TimeoutError) {
      logger.error("Manager", `Analysis timed out after ${DEEPTHINK_TIMEOUTS.MANAGER}ms`);
      return {
        thought_process: "Analysis timed out. Proceeding with direct response.",
        experts: []
      };
    }
    throw e;
...
export const executeManagerReview = async (
  ...
  const reviewTask = async (): Promise<ReviewResult> => {
    const isGoogle = isGoogleProvider(ai);
    ...
    try {
      const resp = await withRetry(
        ...
        RETRY_CONFIG.NETWORK_ERROR.backoffMultiplier,
        (status) => {
          logger.info("Manager", `Review retry attempt ${status.attempt}/${status.maxRetries}`, {
            errorType: status.errorType,
            nextRetryDelay: status.nextRetryDelay
          });
        }
  ...
  try {
    return await withTimeout(reviewTask(), DEEPTHINK_TIMEOUTS.MANAGER, 'manager');
  } catch (e) {
    if (e instanceof TimeoutError) {
      logger.error("Manager", `Review timed out after ${DEEPTHINK_TIMEOUTS.MANAGER}ms`);
      return { satisfied: true, critique: "Review timed out. Proceeding to synthesis." };
    }
    throw e;
  }
};
